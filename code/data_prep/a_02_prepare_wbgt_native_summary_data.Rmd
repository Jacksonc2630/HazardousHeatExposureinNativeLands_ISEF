---
title: "Prepare WBGT native summary data"
...
## Load native WBGT file
```{r}
dat.wbgt = readRDS(paste0(wbgt.folder,'weighted_area_raster_native_wbgtmax_daily_',start_year_wbgt,'_',end_year_wbgt,'.rds'))
```
...
dat.wbgt.summarised = dat.wbgt %>%
  ...
  group_by(native_id, year) %>%
  dplyr::summarise(wbgt_26=sum(wbgt_26), wbgt_28=sum(wbgt_28), wbgt_30=sum(wbgt_30), wbgt_35=sum(wbgt_35))
```

## Save file over time
```{r}
saveRDS(dat.wbgt.summarised,paste0(wbgt.folder,'weighted_area_raster_native_wbgtmax_daily_',start_year_wbgt,'_',end_year_wbgt,'_over_time.rds'))
```
...
dat.wbgt.summarised.start = dat.wbgt.summarised %>%
  ...
  dplyr::select(-year) %>%
  dplyr::rename_with(~ paste0(., "_", start_year_wbgt), -native_id)

dat.wbgt.summarised.end = dat.wbgt.summarised %>%
  filter(year%in%c(end_year_wbgt)) %>%
  dplyr::select(-year) %>%
  dplyr::rename_with(~ paste0(., "_", end_year_wbgt), -native_id)
...
dat.wbgt.summarised.last.5 = dat.wbgt.summarised %>%
  ...
  dplyr::rename_with(~ paste0(., "_", as.numeric(end_year_wbgt-4), "_", end_year_wbgt), -native_id)
```
...
dat.wbgt.summarised.regression.26 = dat.wbgt.summarised %>%
  ...
  select(native_id,annual_change_wbgt_26,total_change_wbgt_26)
...
dat.wbgt.summarised.regression.28 = dat.wbgt.summarised %>%
  ...
  select(native_id,annual_change_wbgt_28,total_change_wbgt_28)
...
dat.wbgt.summarised.regression.30 = dat.wbgt.summarised %>%
  ...
  select(native_id,annual_change_wbgt_30,total_change_wbgt_30)
...
dat.wbgt.summarised.regression.35 = dat.wbgt.summarised %>%
  ...
  select(native_id,annual_change_wbgt_35,total_change_wbgt_35)
```
...
```{r}
saveRDS(dat.wbgt.summarised.regression.merged,paste0(wbgt.folder,'weighted_area_raster_native_wbgtmax_daily_',start_year_wbgt,'_',end_year_wbgt,'_regression_analysis.rds'))
```
